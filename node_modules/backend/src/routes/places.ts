...
placesRouter.get("/", async (_req, res, next) => {
  try {
    const result = await pool.query(
      `select id, name, description, location, province, tags, image_url as "imageUrl", created_at as "createdAt", updated_at as "updatedAt"
       from places
...
placesRouter.post("/", async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `insert into places (name, description, location, province, tags, image_url)
       values ($1, $2, $3, $4, $5, $6)
...
placesRouter.put("/:id", async (req, res, next) => {
  try {
    ...
    const current = await pool.query(
      `select id, name, description, location, province, tags, image_url as "imageUrl"
       from places
       where id = $1`,
      [id]
    );
